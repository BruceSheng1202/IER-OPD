...
from slime.utils.metric_utils import compute_bayes_at_n, compute_pass_rate, compute_rollout_step, compute_statistics, dict_add_prefix
...
class RolloutManager:
    ...
    def drain_for_weight_update(self, timeout: float = 120.0):
        """Before a weight update, wait for the background fully-async rollout
        worker to finish every in-flight generation, so pause_generation /
        update_weights does not interrupt an in-progress decode (which would
        make sglang return partial top-logprobs and corrupt student top-k).

        No-op for the non-fully-async rollout path (no persistent background
        worker exists, and sync rollout already finishes generation before
        update_weights). Returns a report dict for logging.
        """
        if "fully_async" not in (self.args.rollout_function_path or ""):
            logger.info("drain_for_weight_update: skipped (not fully_async path)")
            return {"status": "skipped", "reason": "not fully_async"}
        try:
            from fully_async_rollout import drain_for_weight_update as _drain
        except Exception as e:  # pragma: no cover - import path depends on PYTHONPATH
            logger.warning(f"drain_for_weight_update: cannot import fully_async_rollout: {e}")
            return {"status": "skipped", "reason": f"import_error: {e}"}
        return _drain(timeout=timeout)

    def resume_after_weight_update(self):
        """Resume the background fully-async rollout worker after a weight
        update. Counterpart of ``drain_for_weight_update``."""
        if "fully_async" not in (self.args.rollout_function_path or ""):
            return
        try:
            from fully_async_rollout import resume_after_weight_update as _resume
            _resume()
        except Exception as e:  # pragma: no cover
            logger.warning(f"resume_after_weight_update: {e}")
...
def _log_eval_rollout_data(rollout_id, args, data, extra_metrics: dict[str, Any] | None = None):
    ...
    for key in data.keys():
        ...
        if all(reward in (0, 1) for reward in rewards):
            log_dict |= dict_add_prefix(compute_bayes_at_n(rewards, group_size), f"eval/{key}-")
