    ...
    def add_slime_arguments(parser):
        ...
        def add_algo_arguments(parser):
            ...
            parser.add_argument(
                ...
                action="store_true",
                default=False,
                help="Disable all periodic and final training checkpoint writes (benchmarking only).",
            )
        ...
        def add_on_policy_distillation_arguments(parser):
            ...
            parser.add_argument(
                ...
                default=0,
                help=(
                    "Size of the larger top-logprob sampling pool requested from SGLang "
                    "(student rollout + teacher). Defaults to --opd-topk-metrics-k; when set larger, "
                    "the extra log-probabilities are used only to look up true probabilities for tokens outside a "
                    "method's top-k (e.g. IER two-sided union completion). Non-IER methods are unaffected "
            ...
            parser.add_argument(
                ...
                default=-12.0,
                help=(
                    "Log-probability assigned to candidate ids unavailable from one model. "
            ...
            parser.add_argument(
                ...
                type=int,
                default=1,
                help="Minimum per non-empty response, applied after global selection; may exceed the global budget.",
            )
            parser.add_argument(
                "--opd-compat-proxy",
                type=str,
            ...
            # Numerical stability for IER scores used by token selectors.
            parser.add_argument(
                "--ier-eps",
                type=float,
                default=1e-8,
                help="Positive denominator floor for IER Fisher leverage and noise.",
            )
            return parser
        ...
        def add_reward_model_arguments(parser):
            ...
            parser.add_argument(
                ...
                type=str,
                default=None,
                help="OPD teacher SGLang generation endpoint, e.g. http://localhost:8000/generate.",
            )
            parser.add_argument(
                "--custom-rm-path",
                type=str,
...
def slime_validate_args(args):
    ...
    if args.use_opd:
        if args.opd_type not in ("sglang", "megatron"):
            raise ValueError("--opd-type must be 'sglang' or 'megatron' when --use-opd is enabled.")
        ...
        if args.opd_type == "megatron":
            ...
            if args.opd_budget_mask != "full" and args.opd_topk_metrics_k <= 0:
            ...
            if not math.isfinite(args.opd_budget_ratio) or not 0 < args.opd_budget_ratio <= 1:
                raise ValueError("--opd-budget-ratio must be in (0, 1].")
            if args.opd_budget_min_keep_per_sample < 0:
                raise ValueError("--opd-budget-min-keep-per-sample must be nonnegative.")
            if not math.isfinite(args.ier_eps) or args.ier_eps <= 0:
                raise ValueError("--ier-eps must be finite and positive.")
            if not math.isfinite(args.opd_ier_floor_logp):
                raise ValueError("--opd-ier-floor-logp must be finite.")
