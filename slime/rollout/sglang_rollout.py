...
async def generate(args: Namespace, sample: Sample, sampling_params: dict[str, Any]) -> Sample:
    ...
    if topk_enabled(args) and "output_top_logprobs" in output["meta_info"]:
        student_top = output["meta_info"]["output_top_logprobs"]
        # sglang can occasionally return output_top_logprobs shorter than
        # output_token_logprobs when a decode is interrupted (e.g. by
        # pause_generation during a weight update). The response tokens are
        # appended below, so a short top-logprobs would make student top-k
        # shorter than response_length and trip the TIP length check. Pad the
        # tail with None so per-position parsing falls back gracefully instead.
        if len(student_top) < len(new_response_tokens):
            logger.warning(
                f"sglang returned {len(student_top)} top-logprob positions for "
                f"{len(new_response_tokens)} response tokens (partial decode?); "
                f"padding tail with None."
            )
            student_top = list(student_top) + [None] * (len(new_response_tokens) - len(student_top))
        sample.metadata["student_top_logprobs"] = student_top
...
async def eval_rollout_single_dataset(
    ...
    for _i, prompt_sample in enumerate(dataset.samples):
        for j in range(dataset_cfg.n_samples_per_eval_prompt):
            ...
            sample.metadata.setdefault("data_source", dataset_cfg.name)
    ...
    return {
        dataset_cfg.name: {
            ...
            "samples": data,
            "n_samples_per_prompt": dataset_cfg.n_samples_per_eval_prompt,
        }
    }
