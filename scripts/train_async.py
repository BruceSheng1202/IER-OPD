...
# Asynchronous training with synchronized rollout draining at weight updates.
def train(args):
    ...
    for rollout_id in range(args.start_rollout_id, args.num_rollout):
        ...
        if not args.disable_checkpoint_save and should_run_periodic_action(
        ...
        if (rollout_id + 1) % args.update_weights_interval == 0:
            ...
            # Fully-async: drain the background worker before pushing weights.
            # Without this, an in-flight decode crosses the pause_generation /
            # update_weights window and sglang returns partial output_top_logprobs
            # (shorter than output_token_logprobs), corrupting student top-k.
            ray.get(rollout_manager.drain_for_weight_update.remote())
            actor_model.update_weights()
            rollout_manager.resume_after_weight_update.remote()
